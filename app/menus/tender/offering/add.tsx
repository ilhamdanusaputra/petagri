...
export default function TenderOfferingAdd() {
	...
	const updateProduct = (index: number, patch: any) =>
		setProducts((p) => p.map((it, i) => (i === index ? { ...it, ...patch } : it)));
	const addProduct = () =>
		setProducts((p) => [...p, { product_name: "", dosage: "", qty: 1, price: null, note: "" }]);
	const removeProduct = (index: number) => setProducts((p) => p.filter((_, i) => i !== index));
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (rows.length > 0) {
				const { error: prodErr } = await supabase.from("tender_offerings_products").insert(rows);
		...
		<ThemedView style={{ flex: 1 }}>
			<ScrollView contentContainerStyle={{ padding: 16 }}>
				...
				<View style={{ marginTop: 12 }}>
					<ThemedText style={{ fontWeight: "600", marginBottom: 8 }}>Produk Dibutuhkan</ThemedText>
					...
					{assignProducts.length === 0 ? (
						...
						<View style={{ marginBottom: 8 }}>
							{assignProducts.map((p: any) => (
								<ThemedText key={p.id} style={{ color: "#374151", marginBottom: 4 }}>
									- {p.product_name}
					...
					<ThemedText style={{ fontWeight: "600", marginBottom: 8 }}>Produk (Ajukan)</ThemedText>
							...
							<View style={styles.productRow}>
								<View style={{ flex: 1 }}>
									<TextInput
										...
										onChangeText={(t) => updateProduct(index, { product_name: t })}
								...
								<TextInput
									...
									onChangeText={(t) => updateProduct(index, { qty: Number(t || 0) })}
								/>
								<TextInput
									...
									onChangeText={(t) => updateProduct(index, { price: t ? Number(t) : null })}
								/>
								<Pressable onPress={() => removeProduct(index)} style={styles.deleteBtn}>
