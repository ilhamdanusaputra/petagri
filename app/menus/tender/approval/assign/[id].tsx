		...
		const load = async () => {
			...
			try {
				...
				const { data: approveData, error: approveErr } = await supabase
					...
					.eq("tender_assign_id", assignId)
					.maybeSingle();
				if (!approveErr && approveData && approveData.winning_tender_offering_id) {
					setWinnerId(approveData.winning_tender_offering_id as string);
				}

				// fetch profile names for offered_by ids via view v_profiles
				const offeredByIds = Array.from(new Set(offersList.map((o) => o.offered_by))).filter(
					Boolean,
				);
	...
	const handlePickWinner = async (offeringId: string) => {
		...
		try {
			...
			if (existsData && existsData.id) {
				...
				const { error: insErr } = await supabase.from("tender_approves").insert([
					{
						...
						renderItem={({ item }) => {
							...
							return (
								<View
									style={[
										styles.card,
										isWinner ? { backgroundColor: "#ECFDF5", borderColor: "#86EFAC" } : {},
									]}>
									<View style={{ flex: 1 }}>
										<ThemedText style={{ fontWeight: "600" }}>{name}</ThemedText>
										<View style={{ marginTop: 6 }}>
											{item.tender_offerings_products?.length > 0 ? (
												item.tender_offerings_products.map((p: any, idx: number) => (
													<ThemedText
														key={`${item.id}-${idx}`}
														style={{ color: "#6B7280", marginTop: 4 }}>
														- {p.product_name}
														{p.dosage ? ` (${p.dosage})` : ""}
														{p.price != null ? ` — Rp ${formatPrice(p.price)}` : ""}
													</ThemedText>
												))
											) : (
