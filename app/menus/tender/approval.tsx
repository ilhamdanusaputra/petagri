...
export default function TenderApproval() {
	...
	return (
		<ThemedView style={styles.container}>
			...
			<ApprovalSectionHeader title="Assigned — Belum Ada Pemenang" count={withoutWinner.length} />
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<ApprovalCard
							assign={item}
							showDeadline={true}
							onPress={() => router.push(`/menus/tender/approval/assign/${item.id}`)}
						/>
					)}
				/>
			)}

			<ApprovalSectionHeader title="Assigned — Sudah Ada Pemenang" count={withWinner.length} />
			{withWinner.length === 0 ? (
				<ThemedText style={{ color: "#6B7280", marginTop: 8 }}>
