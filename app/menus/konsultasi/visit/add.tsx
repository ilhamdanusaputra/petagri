			...
			<ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
				...
				<View style={styles.fieldGroup}>
					...
					<Pressable
						style={[
							styles.input,
							{ backgroundColor: inputBg, borderColor: border, justifyContent: "center" },
						]}
						onPress={() => setShowFarmPicker(true)}>
						<ThemedText style={{ color: formData.farmId ? text : placeholderColor }}>
							{formData.farmId
				...
				<Modal visible={showFarmPicker} transparent animationType="slide">
					<Pressable style={styles.modalOverlay} onPress={() => setShowFarmPicker(false)}>
						<View style={[styles.modalContent, { backgroundColor: cardBg, borderColor: border }]}>
							...
							/>
							<Pressable style={styles.modalClose} onPress={() => setShowFarmPicker(false)}>
								<ThemedText style={{ color: "#fff", fontWeight: "600" }}>Tutup</ThemedText>
							</Pressable>
						</View>
					</Pressable>
				</Modal>
				...
				<View style={styles.fieldGroup}>
					...
					<Pressable
						...
						<ThemedText style={{ color: formData.consultantId ? text : placeholderColor }}>
							{formData.consultantId
								? konsultans.find((k) => k.id === formData.consultantId)?.full_name ||
				...
				<Modal visible={showConsultantPicker} transparent animationType="slide">
					<Pressable style={styles.modalOverlay} onPress={() => setShowConsultantPicker(false)}>
						<View style={[styles.modalContent, { backgroundColor: cardBg, borderColor: border }]}>
							...
							<FlatList
								...
								renderItem={({ item }) => (
									<Pressable
										...
										<ThemedText style={{ color: text }}>{item.full_name || item.email}</ThemedText>
									</Pressable>
								)}
							/>
							<Pressable style={styles.modalClose} onPress={() => setShowConsultantPicker(false)}>
								<ThemedText style={{ color: "#fff", fontWeight: "600" }}>Tutup</ThemedText>
							</Pressable>
						</View>
					</Pressable>
				...
				<View style={styles.fieldGroup}>
					...
					/>
					<ThemedText style={{ color: placeholderColor, fontSize: 12, marginTop: 4 }}>
						Format: 2026-01-25
					</ThemedText>
				</View>
				...
				<View style={styles.fieldGroup}>
					...
					<TextInput
						style={[styles.input, { backgroundColor: inputBg, color: text, borderColor: border }]}
						value={formData.scheduledTime.toTimeString().split(" ")[0].substring(0, 5)}
						onChangeText={(value) => {
